...
// For helper funtion to create a file
#include <iostream>
#include <fstream>
...
{
    std::cout << "TOP: " << topologyFilename << std::endl;
...
void Net::openFile() {
    outputFile.open("/home/shared/TAMU_AutoDrive_Year_1/catkin_ws/src/stop_signs/bucket/image.txt");
}
void Net::closeFile(){
    outputFile.close();
}

// Helpef function to create a file name containing all of the stopsigns
void Net::addNameToFile( string &filename, bool stop, float output)  {
    if (stop) {
...
//
void Net::reportResults( Sample &sample) 
{
    ...
    for (auto &n : layers.back()->neurons[0]) { // For all neurons in output layer
        info << n.output << " ";
        if (n.output >= 0.0)
            addNameToFile(sample.imageFilename, true, n.output);
        else
            addNameToFile(sample.imageFilename, false,n.output);
