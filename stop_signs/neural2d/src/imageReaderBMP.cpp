...
{
    std::string const fname = "/home/shared/TAMU_AutoDrive_Year_1/catkin_ws/src/stop_signs/bucket/image.bmp";
    ...
    // Check to see if the + 3 messes up things
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            if (colorChannel == NNet::R) {
            ...
            } else if (colorChannel == NNet::M) {
                red = imageData[x * 3 + 2]; // Red
                green = imageData[x * 3 + 1]; // Green
                blue = imageData[x * 3 + 0]; // Blue
                if (red-green>20 and red-blue>20 and red>50){
                    if (green>100 and blue > 100 and (green-blue)<15)
                        val = 128;
                    else
                        val = 255;
                }
                ...
                // green = imageData[x * 3 + 1];
                // std::cout << "Green " << val << std::endl;
                // glue = imageData[x * 3 + 0];
                // std::cout << "Blue " << val << std::endl;
                // val = imageData[x * 3 + 2]; // Blue
